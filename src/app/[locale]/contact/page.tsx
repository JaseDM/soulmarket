...
// Asegura lectura de envs en runtime
export const dynamic = 'force-dynamic'

export default async function ContactPage({
  params
}: {
  // ✅ En tu proyecto, Next tipa params como Promise
  params: Promise<{locale: string}>
}) {
  const {locale} = await params
  setRequestLocale(locale)

  // Carga SOLO los mensajes de este namespace
  const contactMsgs = (await import(`@/locales/${locale}/Contact.json`))
    .default as AbstractIntlMessages

  // Traductor local con el namespace Contact (ensancha tipo para evitar 'never')
  const t = createTranslator({
    locale,
    messages: { Contact: contactMsgs },
    namespace: 'Contact'
  }) as (key: string, values?: Record<string, unknown>) => string

  const brand = 'SoulMarket'

  // Site Key en servidor (runtime) → prop para el Client Component
  const siteKey =
    process.env.NEXT_PUBLIC_TURNSTILE_SITE_KEY ??
    process.env.TURNSTILE_SITE_KEY_PUBLIC ??
    ''

  if (!siteKey) {
    console.warn('[contact/page] Falta la site key de Turnstile en producción')
  }

  return (
    <main className="relative min-h-screen bg-background-3 dark:bg-background-5 text-stroke-9 dark:text-stroke-1">
      {/* === HERO ======================================================= */}
      <section className="relative overflow-hidden pt-[160px] pb-16 lg:pb-[100px]">
        <div
        ...
        <div className="grid items-start gap-10 lg:grid-cols-2">
          <div id="form" className="relative">
            ...
            >
              ...
              {/* siteKey por props desde servidor */}
              <ContactForm i18n={contactMsgs.form as ContactFormI18n} siteKey={siteKey} />
            </div>
          </div>
